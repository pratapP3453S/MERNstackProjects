...
export const getSimilarProducts = (category) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8000/api/v1/products?category=${category}`);
...
export const getSliderProducts = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("http://localhost:8000/api/v1/products/all");
    console.log(data);
    dispatch({
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:8000/api/v1/admin/reviews?id=${id}`);
